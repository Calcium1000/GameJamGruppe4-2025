...
{
    private Vector2
        movement; // Man skal kunne gemme det fra "Vector2" som kommer ind når brugeren trykker på knappen på skærmen ind på movement
    ...
    private void
        OnMovement(InputValue value) // Her laver man en funktion som holder øje med Input systems value, altså dens værdi
    {
        movement = value
            .Get<
                Vector2>(); // Her bliver movement sat til Vector2 fra Input Action når brugeren trykker på WASD-knapperne

        if (movement.x != 0 || movement.y != 0)
        {
            // Her bliver value.vector2 sat til [0,0] når WASD ikke bliver trykket på
        }
    }
